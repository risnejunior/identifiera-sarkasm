...
sample_count = 1000 #36366 #how many of both class3es of samples to use, to ensure they are 50/50

rel_data_path = os.path.join(".","..", "datasets","poria")
path_name_normal = os.path.join(rel_data_path, "balanced_cleaned","normal") #39'967
path_name_sarcastic = os.path.join(rel_data_path, "balanced_cleaned","sarcastic") #36'366 # sum: 76'333
...
if (print_debug) : print( str( len(file_list_all) ) + " files selected")
#### functions ###############################################################################
...
def tokenize_helper(path_name, file_list, samples, all_words, normal_texts, sarcastic):
	for file_name in file_list:
		...
		samples[file_name] = {'sarcastic': True, 'text': text_tokens, 'int_vector':[]}
...
if (print_debug) : print( str( len(samples) ) + " samples tokenized")
...
file_name = os.path.join(rel_data_path, 'processed.json')
...
file_name = os.path.join(rel_data_path, 'vocabulary.json')
with open(file_name, 'w', encoding='utf8') as out_file:
